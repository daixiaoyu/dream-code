 ...
 * (the number of buckets equals to bucketCount) or arbitrary spacing based on  custom bucket
 * boundaries supplied by the user via an array of double values. The result type of both variants is slightly
 * different, the first function will return a tuple consisting of two arrays. The first array contains the
 * computed bucket boundary values and the second array contains the corresponding count of values (i.e. the
 * histogram).
 * The second variant of the function will just return the histogram as an array of integers.
 **/
object RDD_22_histogram {
